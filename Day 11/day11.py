...
def updateFacing(currentFacing, direction):
...
if __name__ == "__main__":
    ...
    shipHull = dict()
    shipHullPainted = dict()

    print("Inititalising")
    task1pc = intcodePC(program, [WHITE])
    print("Beginning execution")

    while task1pc.program[task1pc.position] != 99:
        currentColour = shipHull.get((robotPositionX, robotPositionY), BLACK)
        task1pc.inputs.append(currentColour)
        paintColour = task1pc.getNextOutput()

        shipHullPainted[(robotPositionX, robotPositionY)] = True
        shipHull[(robotPositionX, robotPositionY)] = paintColour

        robotFacing = updateFacing(robotFacing, task1pc.getNextOutput())
    ...
    # print(shipHullPainted.keys())

    keys = shipHullPainted.keys()

    result = [['.' for _ in range(45)] for i in range(6)]

    # print(result)
    for key in keys:
        print(key, shipHull[key])
        if shipHull[key] == WHITE:
            result[-1 * key[1]][key[0]] = "#"
        else:
            result[-1 * key[1]][key[0]] = "."
